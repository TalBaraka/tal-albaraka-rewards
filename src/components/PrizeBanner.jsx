...
export default function PrizeBanner({ state, onUpdate }) {
  ...
  const game = state.game_selected ? GAME_BY_KEY(state.game_selected) : null;

  const confirmUse = async () => {
    if (busy) return;
    setBusy(true);
    try {
      const res = await base44.functions.invoke("confirmPrizeUse", {
      ...
      // server enforces validity; keep current view
    } finally {
      setBusy(false);
    }
  };
        ...
        {game ? (
          <>
            <div className="mt-3 text-sm text-white/80">
              لعبتك: <span className="font-bold text-amber-300">{game.name}</span>
            </div>
            <Button
              onClick={confirmUse}
            ...
            >
              {busy ? <Loader2 className="h-5 w-5 animate-spin" /> : "تأكيد الاستعمال"}
            </Button>
          </>
        ) : (
          <Button
  ...
  if (state.prize_status === "used") {
    return (
      <div className="rounded-3xl border border-emerald-400/30 bg-emerald-400/10 p-6 text-center backdrop-blur">
        ...
        {game && (
          <div className="mt-1 text-sm text-white/70">اللعبة: {game.name}</div>
        )}
        {state.prize_used_at && (
          <div className="mt-1 text-xs text-white/50">
            بتاريخ {new Date(state.prize_used_at).toLocaleString("ar-EG")}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="rounded-3xl border border-white/15 bg-white/5 p-6 text-center backdrop-blur">
      ...
      {game && (
        <div className="mt-1 text-sm text-white/60">اللعبة كانت: {game.name}</div>
      )}
    </div>
  );
}